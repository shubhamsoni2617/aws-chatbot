...
import TaskStatusSelect from "../TableMyAssignedTask/TaskStatusSelect";

// type TableRowSelection<T extends object = object> =
//   TableProps<T>["rowSelection"];

interface TablePublicAssignedTaskProps {
...
const TablePublicAssignedTask = (props: TablePublicAssignedTaskProps) => {
  const {
    taskSearchValue,
    currentPage,
    // setCurrentPage,
  ...
  if (props.userNameFilter && props.userNameFilter !== "") {
    filteredData = filteredData.filter(
      (item: any) =>
        item.assigned?.name &&
        item.assigned.name.toLowerCase().includes(props.userNameFilter.toLowerCase())
    );
  }

  // Apply filter for Priority
  if (props.priorityFilter && props.priorityFilter !== "" && props.priorityFilter !== "All") {
    filteredData = filteredData.filter(
      (item: any) => item.priorityValue === props.priorityFilter
    );
  }
  ...
  const dataSource: any[] =
    (monitorTaskData?.assigned_task as EngagementAssignedTask[])
      ...
      ?.filter((elem: any) => {
        ...
        return elem?.kpi?.toLowerCase() === kpi?.toLowerCase();
      })
      ?.filter((elem: any) => {
        if (departmentId === null) {
          return true;
        }
        return elem?.department_id === departmentId.toString();
      })
      // Add country filter - check for array intersection
      ?.filter((elem: any) => {
      ...
      ?.map<any>((_: EngagementAssignedTask): any => {
        ...
        // console.log("table data mapping",_);
        return {
          key: _.id,
          ...
          taskCompleted:
            _.status === "In Progress" || _.status === "in-progress"
              ...
              ? "Pending"
              : "Completed",
          kpi: _.kpi, // Add KPI to the returned object if needed
        };
      }) || [];
  setTableData(dataSource);
  // const dataSource = monitorTaskData?.assigned_task || [];
  // setTableData(dataSource);
  // Update total items
  ...
  const columns: TableColumnsType<any> = [
    ...
    },
    {
      ...
      width: 130,
      render: (_, record) => {
        // console.log("record send reminder check", _?.taskCompleted==='Completed')
        return(
        <Button
          style={{
            borderRadius: "40px",
  ...
  useEffect(() => {
    console.log("selected row for reminder", selectedRowForReminder)
  },[selectedRowForReminder])
  return (
    <div className="mt-[30px]">
                    ...
                    <div>
                      ...
                      <div className="font-semibold mt-[2px]">
                        <TaskStatusSelect
                          ...
                          onChange={(newValue) =>
                          {
                            if(record.taskCompleted === 'Completed'){
                              message.warning("You cannot change status of a Completed Task");
                              // return
                            }
                            ...
                            console.log("sagaebdfbasfv",record.taskCompleted)
      ...
      <SendReminderModal
        ...
        // preselectedData={selectedRowForReminder}
...
export default TablePublicAssignedTask;
