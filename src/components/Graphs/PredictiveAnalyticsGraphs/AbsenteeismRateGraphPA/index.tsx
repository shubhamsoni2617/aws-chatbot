...
const AbsenteeismRatePA = ({
  ...
  const CustomTooltip = ({ active, payload }: any) => {
    if (active && payload && payload.length) {
      const isHighlighted =
        payload[1]?.payload.quarter_number === getCurrentQuarterNumber() &&
        payload[1]?.dataKey === "2024";
      // const forYear =
      //   payload[1]?.payload.quarter_number <= getCurrentQuarterNumber();
      console.log(
        active,
              ...
              <p
                ...
                style={{
                  color:
                    ...
                    (payload[1]?.payload.quarter_number <= getCurrentQuarterNumber()+1 && payload[1]?.payload.quarter_number >= getCurrentQuarterNumber()) ?index===1? '#c847e8' :'#a5b4ff':
                      entry.color,
                }}
              >
                {
                ...
                //   ? "2025"
                //   : ((payload[1]?.payload.quarter_number <= getCurrentQuarterNumber() &&
                //   payload[1]?.dataKey === "2024" ) && index === 0 && index < getCurrentQuarterNumber()-1)? "2024":
                  
                (payload[1]?.payload.quarter_number <= getCurrentQuarterNumber()+1) ? index===0? '2024': '2025' :entry.dataKey}
                : {entry.value}
              </p>
            );
          })}
            ...
            shape={(props: any) => {
              ...
              return (
                <RoundedBar
                  ...
                  dataKey={key2}
                  colors={
                    //TODO MAKE THIS CONDITION DYNAMIC
                    // (isPredictiveAnalytics &&
                    //   props?.["quarter_number"] > getCurrentQuarterNumber()) ||
                    // (!isPredictiveAnalytics &&
                    //   props?.["quarter_number"] === getCurrentQuarterNumber())
                    //   ? 
                   ( props.index===3 || props.index===2) ? colorPalettes[key3]:colorPalettes[key2]
...
export default AbsenteeismRatePA;
